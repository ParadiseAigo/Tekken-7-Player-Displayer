...
std::string setCommentInLine(char* line, char* comment) {
    std::string originalLine = std::string(line);
    size_t indexComment = originalLine.find_last_of(std::string(PLAYERLIST_COLUMN_DELIMITER)) + 1;
    std::string lineWithComment = originalLine.substr(0, indexComment).append(std::string(comment));
